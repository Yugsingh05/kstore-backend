...
class salesRepo {
  ...
  async CancelSale(id: string) {
    const isSaleCancelled = await this.dbInstance
      .select()
      .from(sales)
      .where((eq(sales.id, id)))


      if(!isSaleCancelled.length){
        return {
          msg: "Sale not found",
          success: false,
        };
    ...
    if (isSaleCancelled.length > 0 && isSaleCancelled[0].Salesstatus === "CANCELLED") {

      return {
        msg: "Sale already cancelled",
        success: false,
      };
    }
